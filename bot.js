...
var nenene = FunGetDataBaseSync("CurrentOrder");
                    // console.log(nenene)
if (nenene != null) {
    CurrentOrder = nenene;
    console.log("đã lấy dữ liệu cux ")
        ...
        if (!CurrentOrder.isOpen) {
            ...
            CurrentOrder.TimeOpen = new Date().getTime();   
             ...
             CurrentOrder.isOpen = true;
               ...
               CurrentOrder.avgEntryPrice  =(CurrentOrder.avgEntryPrice+ pricepull )/2
...
client.addStream('XBTUSD', 'tradeBin5m', data => {
    ...
    if (lastCandle != null && ArDataClient.length != 0 && new Date(DataBucketed[0].timestamp).getTime() != new Date(lastCandle.timestamp).getTime()) {
        ...
        if (DataBucketed.length > 0 && new Date(DataBucketed[0].timestamp).getTime() + 60000 == new Date(lastCandle.timestamp).getTime()) {
            ...
            if (ActiveTrade)
                if (!CurrentOrder.isOpen) {
                    ...
                    if (temp.TG != "__" ) {
                        ...
                        CurrentOrder.dataclose.listtrend.push("cách cũ");
                        ArToast.push({
                            y: price,
                            type: "INFO",
                            name: temp.TG,
                    ...
                    if (temp.TG == "TANG" &&  CurrentOrder.side =="Sell") {
                        bomthem=true;
                    } else   if (temp.TG == "GIAM" &&  CurrentOrder.side =="Buy") {
                        bomthem=true;
                    }
